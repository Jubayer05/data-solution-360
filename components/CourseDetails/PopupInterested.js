export default function PopupInterested({ onClose, onRegister }) {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex justify-center items-center z-50">
      ...
      <div className="relative bg-gradient-to-tr from-purple-600 via-indigo-500 to-indigo-800 text-white rounded-3xl shadow-2xl p-6 sm:p-8 w-[90%] max-w-lg animate-fadeIn scale-95 hover:scale-100 transform transition-all duration-500 ease-in-out">
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-4 sm:gap-6">
          <button
            onClick={onRegister}
            className="w-full sm:w-auto px-4 py-2 sm:px-6 sm:py-3 bg-gradient-to-r from-yellow-400 via-orange-500 to-red-500 text-white font-bold rounded-full shadow-lg transform hover:scale-105 transition-transform duration-300 hover:shadow-yellow-400/50 text-sm sm:text-base"
          >
            Register Now
          </button>
          <button
